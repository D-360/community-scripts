...
var LOGIN_DATA = "username=" + USERNAME + "&password=" + PASSWORD + "&Login=Login&user_token=";
...
function AuthInfo(session, csrf) {
	...
	this.csrf = csrf;
	this.id = ID();
	this.toString = function() { return "{ id: " + this.id + ", session: " + this.session + ", csrf: " + this.csrf + " }" };}
function AuthPool(name) {
	...
	this.pollSession = function() { var e = find(this.sessions, function(e) { return !e.id }); if(e) this.lock(e); return e };
	this.recoverSession = function() { return find(this.sessions, function(e) { return e.id == ID() }) };
	this.releaseSession = function() { var e = find(this.sessions, function(e) { return e.id == ID() }); if(e) this.unlock(e); return this };
...
function sendingRequest(msg, initiator, helper) {
	...
	log("{ method: sendingRequest (out), path: " + msg.getRequestHeader().getPrimeHeader() + " }");
}
...
function responseReceived(msg, initiator, helper) {
	...
	if(invalidationLogic(responseHeader, httpSession, r_code)) {
		...
		log("{ method: responseReceived (session-invalidated), auth-pool: " + authPool + " }");
		authInfo = doAuth(helper);
		if(authInfo)
			authPool.pushSession(authInfo);
 ...
 */
function buildFromMessage(msg, version, secure, uri, escaped, method, data, charset, encode, content_type, cookies, user_agent) {
	...
	if(uri)
		header.setURI(new URI(uri, escaped));
	if(cookies)
		header.setCookies(cookies);
	if(secure === true || secure === false)
		header.setSecure(secure);
	if(user_agent)
...
function buildNewMessage(version, secure, uri, escaped, method, data, charset, encode, content_type, cookies, user_agent) {
	return buildFromMessage(new HttpMessage(), version, secure, uri, escaped, method, data, charset, encode, content_type, cookies, user_agent);
}
...
function doAuthInternal(msg, helper, follow_redirects, retries, note) {
	for(var i = AUTH_INTERNALL_RETRIES; i > 0; --i) {
		...
		if(message != null) {
			...
			try { Thread.sleep(AUTH_INTERNALL_ERR_DELAY) }
			catch(e) {
...
function extractCSRF(msg, csrfFormName, csrfHeaderName) {
	...
	value = extractValue(msg.getResponseBody().toString(), csrfFormName, "meta", "content");
	if(hasValue(value))
		return value;

	value = extractValue(msg.getResponseBody().toString(), csrfFormName, "input", "value");
	if(hasValue(value))
		return value;
...
function replaceCSRF(msg, csrf) {
	...
	log("{ method: replaceAuthInfo, "
		+ "csrf@header: " + (r_count_h ? "set" : "missing")
		+ ", csrf@post: replaced " + r_count_p + " times "
		+ ", csrf@get: replaced " + r_count_g + " times }");
}
...
function replaceHeaderCSRF(msg, csrf) {
	...
	if(n > 0) {
		...
		if(n > 1) log("{ method: replaceHeaderCSRF, warn-header-appears-" + n + "-times }");
	} return n;
}
...
function replaceGetCSRF(msg, csrf) {
	...
	if(ret.count > 0) {
		...
		log("{ method: replaceGetCSRF, warn-csrf-as-get-param-" + ret.count + "-times }");
	} return ret.count;
}
...
function isCSRFRequired(msg) {
	var requestHeader = msg.getRequestHeader();
	return requestHeader.getHeaderValues(CSRF_HEADER_NAME).size() > 0
...
function parseInitiator(initiator) {
	switch(initiator) {
		case HttpSender.ACCESS_CONTROL_SCANNER_INITIATOR: return "ACCESS_CONTROL_SCANNER";
		case HttpSender.ACTIVE_SCANNER_INITIATOR: return "ACTIVE_SCANNER";
		...
		case HttpSender.PROXY_INITIATOR: return "PROXY";
		case HttpSender.SPIDER_INITIATOR: return "SPIDER";
		default: return initiator.name().replace(/_INITIATOR$/,'');
	}
}
